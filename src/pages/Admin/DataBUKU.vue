    ...
    <q-card>
      ...
      >
        <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="judulBuku" :props="props">
                {{ props.row.judulBuku }}
              </q-td>
            ...
            <q-td key="aksi" :props="props">
              <div class="row q-gutter-md">
                <q-btn :to="{ name:'formEditBUKU', params: { id: props.row._id } }" label="Edit" icon="edit" color="warning" unelevated/>
                <q-btn @click="deleteBook(props.row._id)" label="Hapus" icon="delete" color="negative" unelevated/>
              </div>
            </q-td>
          </q-tr>
        </template>
...
export default {
  ...
  data () {
    return {
      columns: [
        { name: 'judulBuku', align: 'left', label: 'Judul Buku', field: 'judulBuku', sortable: true },
  ...
  methods: {
    ...
    deleteBook (id) {
      this.$q.dialog({
        ...
        this.$axios.delete(`book/delete/${id}`)
          .then(res => {
            if (res.data.sukses) {
